...
class MeanFieldGaussianLinear(nn.Module):
	def __init__(self, 
		...
		self.sigma_w = Parameter(torch.log(init_sigma_scale * prior_sigma_w * torch.ones(out_features, in_features)))
		...
		self.sigma_b = Parameter(torch.log(init_sigma_scale * prior_sigma_b * torch.ones(out_features)))
...
class MeanFieldGaussianConv2d(nn.Module):
	def __init__(self,
		...
		self.sigma_w = Parameter(torch.log(init_sigma_scale * prior_sigma_w * torch.ones(out_channels, in_channels, kernel_size, kernel_size)))
		...
		self.sigma_b = Parameter(torch.log(init_sigma_scale * prior_sigma_b * torch.ones(out_channels)))
